...
* `solve` uses the solver in the Solver package to get a solution to the scrambled cube, prints it, and applies it to the cube.
...
For example, the first stage consists of the set of any state of the cube (G<sub>0</sub>), and the set of moves needed to reach the
...
Notice that any state in G<sub>i+1</sub> is also in G<sub>i</sub>; i.e., the set of states in G<sub>i+1</sub> is a subset of the states
in G<sub>i</sub>, for appropriate values of i. Progression is made in this manner until the fifth stage is reached; only one state meets the
requirements for this stage: the solved state.
...
integers can reach values around 2<sup>42</sup>, which Python handles without issue.

With all this in mind, see below for details on each stage:
